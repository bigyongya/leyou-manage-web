    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="delteGoods(props.item.id)">
            <i class="el-icon-delete"/>
          </v-btn>
          <v-btn icon v-if="props.item.saleable" @click="soldOut(props.item.id,props.item.saleable)">下架</v-btn>
          <v-btn icon v-else @click="soldOut(props.item.id,props.item.saleable)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        ...
        oldGoods.skus = await this.$http.loadData("/item/spu/sku/list?id=" + oldGoods.id);
        // 修改标记
        this.isEdit = true;
        // 控制弹窗可见：
        this.show = true;
      ...
      soldOut(id,saleable){
        var self = this;
        this.$http.put("item/spu/soldOut?id="+id+"&saleable="+saleable).then(function (res) {
          self.getDataFromServer();
        })
      },
      delteGoods(id){
        var self = this;
        this.$http.delete("item/spu/deleteGoods?id="+id).then(function (res) {
          self.getDataFromServer();
        })
      }
    },
